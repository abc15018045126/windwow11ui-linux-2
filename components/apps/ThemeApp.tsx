...
const ThemeApp: React.FC<AppComponentProps> = ({ setTitle, onWallpaperChange }) => {
    ...
    return (
        <div className={`p-6 h-full overflow-y-auto custom-scrollbar ${theme.appWindow.background.replace('bg-opacity-60','')} ${theme.appWindow.textColor}`}>
            <div className="flex items-center mb-6">
                <ThemeIcon className="w-10 h-10 text-blue-400 mr-4" />
                <div>
                    <h1 className="text-2xl font-semibold">Themes</h1>
                    <p className="text-sm opacity-80">Change the look and feel of your desktop.</p>
                </div>
            </div>
