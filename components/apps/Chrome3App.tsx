...
const Chrome3App: React.FC<AppComponentProps> = ({ setTitle: setWindowTitle, appInstanceId }) => {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            window.electronAPI?.clearProxyForSession(partition)
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-800 text-white select-none">
            <div className="flex-shrink-0 flex items-center p-1.5 bg-zinc-800 border-b border-zinc-700 space-x-1">
                ...
                <button onClick={() => webviewRef.current?.loadURL('https://www.google.com')} className="p-1.5 rounded-full hover:bg-zinc-700 disabled:opacity-30" title="Home"><HomeIcon /></button>
                <input
                    type="text"
                    value={inputValue}
