...
const MenuDropdown: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => {
...
const MenuItem: React.FC<{ onClick: () => void; children: React.ReactNode, disabled?: boolean }> = ({ onClick, children, disabled }) => {
    return (
        <button onClick={onClick} disabled={disabled} className="w-full text-left px-3 py-1 text-sm hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-sm">
            {children}
        </button>
    );
};
...
const NotebookApp: React.FC<AppComponentProps> = ({ setTitle, initialData }) => {
    ...
    const [statusBarInfo, setStatusBarInfo] = useState<StatusBarInfo>({ line: 1, column: 1, charCount: 0, selectedCount: 0 });
    ...
    const updateStatusBar = useCallback(() => {
        ...
        setStatusBarInfo({ line: currentLineNumber, column: currentColumnNumber, charCount: content.length, selectedCount: selectedCount });
    }, [content]);
    ...
    const zoomIn = useCallback(() => setZoomLevel(prev => Math.min(prev + 10, 500)), []);
    const zoomOut = useCallback(() => setZoomLevel(prev => Math.max(prev - 10, 10)), []);
        ...
        <div className="flex flex-col h-full bg-[#1e1e1e] text-zinc-200">
            <div className="flex-shrink-0 flex items-center space-x-1 px-1 py-0.5 bg-[#2d2d2d] border-b border-zinc-800">
                ...
                <MenuDropdown title="View">
                    <div className="w-full text-left px-3 py-1 text-sm hover:bg-blue-600 flex justify-between items-center cursor-pointer rounded-sm" onClick={toggleWordWrap}>
                        <span>Word Wrap</span>
                        <span>{wordWrap ? '✓' : ''}</span>
                    </div>
                ...
                <input ref={fileInputRef} type="file" accept=".txt,.md,.js,.ts,.html,.css,.json" className="hidden" onChange={handleFileChange} />
            </div>
